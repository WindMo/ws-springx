...
public abstract class FrequencyRestrictServiceTests {
    ...
    void restrictorTest() throws InterruptedException {
        ...
        // 频控测试
        SpringxAppTests.runInDuration(duration, TimeUnit.SECONDS, buildTrigger(restrictor, refer, frequency));
        TimeUnit.SECONDS.sleep(duration);
        Assertions.assertFalse(restrictor.tryRestrict(refer));

        // 重置测试
        String newRefer = refer + random.nextInt(100);
        SpringxAppTests.runInDuration(duration, TimeUnit.SECONDS, buildTrigger(restrictor, newRefer, frequency));
