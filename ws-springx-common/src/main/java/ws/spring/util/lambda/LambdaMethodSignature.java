...
class LambdaMethodSignature {
     ...
     * @return
     */
    static LambdaMethodSignature parseMethodSignature(String methodSignature) throws IllegalMethodSignature {

        String message = String.format("Invalid lambda impl method signature[%s]", methodSignature);
        IllegalMethodSignature.toAssert(methodSignature.startsWith("("), message);
        int separatedIndex = Objects.requireNonNull(methodSignature).indexOf(')');
        IllegalMethodSignature.toAssert(separatedIndex != -1, message);
        ...
        if (parameterTypeSignature.isEmpty()) {
            ...
            for (char c : chars) {
                ...
                if (refType.length() == 0) {
                    ...
                    if (c == 'L') {
                        ...
                        IllegalMethodSignature.toAssert(void.class != type, "The method parameter type cannot be void");
            ...
            IllegalMethodSignature.toAssert(refType.length() == 0, message);
        ...
        IllegalMethodSignature.toAssert(StringUtils.hasText(returnTypeSignature), message);
     ...
     * @return
     */
    private static Class<?> parseTypeSignatureClass(String typeSignature) throws IllegalMethodSignature {
        ...
        if (StringUtils.hasText(typeSignature)) {
            ...
            if (typeSignature.length() == 1) {
                ...
                IllegalMethodSignature.toAssert(StringUtils.hasText(returnTypeClassName), String.format("Invalid type signature[%s]", typeSignature));
                try {
                    return ClassUtils.forName(returnTypeClassName, null);
                } catch (ClassNotFoundException e) {
                    throw new IllegalMethodSignature(String.format("Not found class of type[%s]", returnTypeClassPath));
                }
            }
        }
        throw new IllegalMethodSignature(String.format("Invalid type signature[%s] of lambda implemented method", typeSignature));
    }
     ...
     * @return
     */
    private static Class<?> parsePrimitiveTypeSignatureClass(char signature) throws IllegalMethodSignature {

        Class<?> type = SIGNATURE_PRIMITIVE_TYPE_MAPPING.get(signature);
        if (type == null) {
            throw new IllegalMethodSignature(String.format("Invalid primitive type signature[%s] of lambda implemented method", signature));
        }
        return type;
    }
